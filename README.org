...
sudo apt install fd-find ripgrep libtool-bin cmake
...
# Add this to your .zshrc or .bashrc (to improve SPC v (terminal))
if [[ -v PROOT ]]; then
   cd $PROOT
fi
#+END_SRC
...
* After Install
Create a file on =user.el= with at least this:

#+begin_elisp elisp

;; Locals where emacs can find your projects, use SPC p d to load the projects
(setq projectile-project-search-path '("~/Projetos" "~/Study"))

#+end_elisp

Just change the strings with your project folders path

After that, just press SPC p d to load your projects into emacs.   Use =SPC p p= to enter on a project.
