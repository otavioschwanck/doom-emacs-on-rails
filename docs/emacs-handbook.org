* Quick Tips
- ~SPC s i~ or ~;~ = Quick Search Headers (also works on programming languages).  Use C-n and C-p to go forward / backward on the search buffer.
...
| C-p   | Complete text from all buffers (works everywhere). |
|-------+----------------------------------------------------|

About the C-p, it works everywhere, on search, on find-file, etc.
