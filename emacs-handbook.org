...
| S-RET | Newline while autocomplete open (ignores autocomplete) |
...
| M-1 to M-9 | Navigate between harpoon files by order |
...
| C-1 to C-9 (or SPC 1 to SPC 9) | Switch between workspaces, the template create a new workspace for you. |
