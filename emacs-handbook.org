...
To add some elisp code of your ownm, visit [[file:user-settings.el][user-settings.el]] (SPC f m).  It is the same as config.el for normal doom.
[[file:user-settings.el][user-settings.el]] has a lot of cool stuff and examples of stuff like disabling rubocop, adding commands to terminal (SPC o t), etc.
