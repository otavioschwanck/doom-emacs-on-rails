...
(defun order-snippet-at-top-when-matches (candidates)
  ...
  (interactive)
  (if (and (not (eq major-mode 'text-mode)) (not (eq major-mode 'org-mode)))
        ...
        (dolist (candidate candidates)
          (let ((stripped-candidate (substring-no-properties candidate))
            ...
            (if (and (string= word stripped-candidate) (eq candidate-backend 'company-yasnippet))
              ...
              (when (order-snippet-at-top-when-matches--can-push word stripped-candidate candidate-backend new-list) (push candidate new-list)))))
