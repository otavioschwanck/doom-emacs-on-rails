...
(after! inf-ruby
  ...
  (defun inf-ruby-add-keybindings ()
    (if (cl-search "*rails" (buffer-name))
        (progn
          (message "Ruby Console Tips: Press C-h to send exit, C-M-h to reload, press A to move from normal to insert mode at end, Press C-M-o to disable SQL log."))
      (progn
        (message "Debugging Tips: Press C-h to send continue, press A to move from normal to insert mode at end, C-l to next and C-M-l to step.")))

    (evil-local-set-key 'normal (kbd "A") #'inf-ruby-goto-insert)

    (setq-local logger-disabled nil)

    (evil-local-set-key 'normal (kbd "C-h") #'inf-ruby-exit)
    (define-key evil-insert-state-local-map (kbd "C-h") #'inf-ruby-exit)

    (evil-local-set-key 'normal  (kbd "C-M-h") #'inf-ruby-reload)
    (define-key evil-insert-state-local-map (kbd "C-M-h") #'inf-ruby-reload)

    (evil-local-set-key 'normal  (kbd "C-M-l") #'inf-ruby-step)
    (define-key evil-insert-state-local-map (kbd "C-M-l") #'inf-ruby-step)

    (evil-local-set-key 'normal (kbd "C-l") #'inf-ruby-next)
    (define-key evil-insert-state-local-map (kbd "C-l") #'inf-ruby-next)
