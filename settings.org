...
(defun order-snippet-at-top-when-matches (candidates)
  "Order the snippets / text depending of priority. CANDIDATES: Company candidates."
  (interactive)
  (let* ((matching-snippet nil)
         (matching-text nil)
         (new-list '())
         (word (or (word-at-point t) "")))
    (dolist (candidate candidates)
      (let ((stripped-candidate (substring-no-properties candidate))
        ...
        (if (and (string= word stripped-candidate) (eq candidate-backend 'company-yasnippet))
            (setq matching-snippet candidate)
          (if (and (string= word stripped-candidate) (eq matching-text nil))
              (setq matching-text candidate) (when (order-snippet-at-top-when-matches--can-push word stripped-candidate candidate-backend new-list) (push candidate new-list))
              ))))
    (setq new-list (reverse new-list))
    (unless yas--active-snippets
      (when matching-text (setq new-list (insert-into-list new-list matching-text 0)))
      (when matching-snippet (setq new-list (insert-into-list new-list matching-snippet 0))))
    new-list))

(defun order-snippet-at-top-when-matches--can-push (word stripped-candidate candidate-backend new-list)
  "Verify if can push to autocomplete list."
  (let* ((case-fold-search nil)
         (valid-case (string-match-p word stripped-candidate))
         (exist-in-list (-contains? (mapcar 'substring-no-properties new-list) stripped-candidate))
         (begins-with-two-dots (string= (substring stripped-candidate 0 1) ":"))
         (is-yas yas--active-snippets))
    (if is-yas
        (and valid-case
             (not exist-in-list)
             (not begins-with-two-dots)
             (not (eq candidate-backend 'company-yasnippet))
             (not (eq candidate-backend 'company-dabbrev-code)))
      (and valid-case (not exist-in-list)))))

(after! company
  (setq +lsp-company-backends '(:separate company-capf company-dabbrev-code company-yasnippet))
  (add-to-list 'company-transformers
               #'order-snippet-at-top-when-matches))
...
(map! :after ruby-mode :map ruby-mode-map :i "C-S-e" #'otavio/grb)
#+end_src
