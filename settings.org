...
(after! inf-ruby
  ...
  (defun inf-ruby-add-keybindings ()
    (if (cl-search "*rails" (buffer-name))
        (progn
          (message "Ruby Console Tips: Press C-RET to send exit, C-M-RET to reload, press A to move from normal to insert mode at end, Press C-M-o to disable SQL log."))
      (progn
        (message "Debugging Tips: Press C-RET to send continue, press A to move from normal to insert mode at end, C-l to next and C-M-l to step.")))
